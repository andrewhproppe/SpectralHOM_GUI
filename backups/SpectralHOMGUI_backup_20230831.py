...
from modules.LedIndicatorWidget import *
...
class Ui(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        uic.loadUi('SpectralHOMGUI.ui', self, package='modules') # Load the .ui file
        ...
        # self.graph_update_interval = 5
        # self.graph_update_interval_tracker = 0
        # self.log_time = [0]
        # self.log_pos = [0]
        # self.time_window = float(self.timeWindow.text())
        # self.stage_update_interval = float(self.stageRefreshRate.text())
        ...
        # acceleration = 1e-3
        # max_velocity = 2e-3
        # self.log_pos = self.s.get_position()*self.scale
    ...
    def move_to_t0(self):
        self.s.move_to(self.stage_time_zero.value()/self.scale)
    ...
    def update_tpx_scan_info(self):
        ...
        est_duration = (self.tpx_scan_acq_time.value() + self.tpx_scan_pause_time.value()) * self.tpx_scan_nsteps.value()
        self.tpx_step_size.setText(str(step_size.__round__(ROUND)))
        self.tpx_scan_duration.setText(str(est_duration.__round__(ROUND)))
    ...
    def update_stage_pos(self):
        while True:
            try:
                pos = round(self.s.get_position()*self.scale, ROUND)
                self.current_stage_position = pos
                self.stagePosition.setText(str(pos))
                self.is_moving = self.s.is_moving()
    ...
    def chart_counts(self):
        ...
        if self.chartCounts_btn.isChecked():
            ...
            self.counter = TimeTagger.Counter(tagger=self.tagger, channels=self.active_channels, binwidth=self.ctr_binwidth, n_values=self.ctr_nbins)
        ...
        while True:
            try:
                if hasattr(self, 'counter'):
                    ...
                    norm_channels = []
                    norm_data = []
                    if self.ch1_norm_checkBox.isChecked():
                        ch1_norm = data[0, :]/(data[0, :]+data[1, :]+1e-5)
                        norm_data.append(ch1_norm)
                        norm_channels.append(1)
                    if self.ch2_norm_checkBox.isChecked():
                        ch2_norm = data[1, :]/(data[0, :]+data[1, :]+1e-5)
                        norm_data.append(ch2_norm)
                        norm_channels.append(2)

                    self.chart_counts_graph.plot(t, data, self.active_channels, norm_data, norm_channels)
    ...
    def stage_scan(self):
        ...
        for i, step in enumerate(stage_steps):
            ...
            self.s.move_to(step/self.scale)
            self.is_moving = True
            ctr = 0
    ...
    def hom_scan(self):
        ...
        time.sleep(0.1)
        self.s.move_to(hom_steps[0]/self.scale)
    ...
    def tpx_scan(self):
        ...
        while True:
            ...
            self.accel_val.setValue(1)
            time.sleep(0.1)
            self.s.move_to(tpx_steps[0]/self.scale)
    ...
    def integrated_g2(self):
        ...
        while True:
            if not self.stop_intg2_thread:
                # Start correlation for this stage position
                # Pause to acquire
...
class CountsGraph(PlotCanvas):
    ...
    def plot(self, t, data, channels, norm_data=None, norm_channels=None):
        ...
        for i, d in enumerate(data):
            self.axes.plot(t, d, color=self.colors[channels[i]-1], label=channels[i])
...
class Integrated_g2_Graph(PlotCanvas):
    ...
    def plot(self, t, data):
        ...
        # ymin = np.min(data) * 0.99
        ymin = 0
        ymax = np.max(data) * 1.01
